...
3. Read the `source_commit` values in the mapped public MDX pages. Compare mapped source files between the oldest recorded commit and current `origin/main` of `Crush-AP-app/crush-ap-physics-v2`.
...
13. Run `CRUSH_AP_SOURCE_ROOT="/Users/kanishkjain/Documents/code/Crush AP/crush-ap-v2" ruby scripts/validate-docs.rb` and the available Mintlify broken-link check.
14. If content changed, create or reuse one branch named `codex/automated-docs-sync` in the documentation repository. Commit and push the validated changes, then open or update one pull request into `main`. Never merge it.
15. The pull request must list the source commit, source paths read, pages checked, semantic changes, validation results, and any uncertainty. Avoid stylistic churn and duplicate pull requests.
